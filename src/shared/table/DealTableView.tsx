...
export const stageLabel = (stage: Deal['stage']) => {
	switch (stage) {
		case DealStage.New:
			return (
...
const stageLabel1= (stage: DealStage) => {
	switch (stage) {
		case DealStage.New:
			return 'Новый';
		case DealStage.InProgress:
...
const DealTableView: React.FC<DealTableViewProps> = ({ deals }) => {
	return (
		<Card className="bg-black border-gray-800 text-white">
			<CardHeader  className="flex flex-row items-center justify-between" >
				<CardTitle>Список сделок
...
{deals.length === deals.length && `(${deals.length})`}
			...
			<CardContent className="flex flex-col gap-4">
				{deals.map((deal, idx) => (
					<div key={`${deal.company}-${deal.name}-${idx}`} className="flex gap-4 justify-between border border-gray-800 rounded-lg items-center hover:bg-gray-800/50 transition-colors p-2 cursor-pointer ">
							<div
								key={deal.name}
