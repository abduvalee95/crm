...
const EffectiveMeneger = ({ searchTerm = '', statusFilter = 'all' }: EffectProps) => {
	const filteredAndSortedClients = useMemo(() => {
		let filtered = analyticsData.clientPerformance;
							...
							>
								<div className="flex-1">
									<div className="flex items-center justify-between mb-1">
										...
										<span className="text-green-400 font-semibold">₽{client.revenue.toLocaleString()}</span>
									</div>
									<span className="text-gray-300 text-sm">
										{client.company} • {client.deals} сделок
									</span>
								</div>
								<div className="ml-4">
									<span
									...
									>
										{client.status === 'active' ? 'Активный' : client.status === 'new' ? 'Новый' : 'Неактивный'}
									</span>
								</div>
							</div>
						))}
